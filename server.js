...
*
*   Changes:
*   - res.sendfile -> res.sendFile with absolute path
*   - io.once -> io.on (so handlers register for every new connection, not just the first)
*   - Added path.join for express.static
*/
...
app.get('/', function(req, res) {
  res.sendFile(path.join(__dirname, 'energy_marketplace.html'));
});

// Socket.IO handlers registered ONCE at startup
io.on('connection', function(socket) {
  console.log('[MAIN SERVER] Browser connected');
  ...
  // Receive token counts from producers
  socket.on('display_tokens_0', function(data) { energy_token[0] = data; });
  socket.on('display_tokens_1', function(data) { energy_token[1] = data; });
  socket.on('display_tokens_2', function(data) { energy_token[2] = data; });
  socket.on('display_tokens_3', function(data) { energy_token[3] = data; });
  socket.on('display_tokens_4', function(data) { energy_token[4] = data; });
  socket.on('display_tokens_5', function(data) { energy_token[5] = data; });
});

// Update marketplace every 1s
setInterval(function() {
  io.emit('update_marketplace', {
    add0: producer_address_list[0], add1: producer_address_list[1], add2: producer_address_list[2],
    add3: producer_address_list[3], add4: producer_address_list[4], add5: producer_address_list[5],
    bid0: asking_bid[0], bid1: asking_bid[1], bid2: asking_bid[2],
    bid3: asking_bid[3], bid4: asking_bid[4], bid5: asking_bid[5],
    add6: consumer_address_list[0], add7: consumer_address_list[1], add8: consumer_address_list[2],
    add9: consumer_address_list[3], add10: consumer_address_list[4], add11: consumer_address_list[5],
    tok0: energy_token[0], tok1: energy_token[1], tok2: energy_token[2],
    tok3: energy_token[3], tok4: energy_token[4], tok5: energy_token[5]
  });
}, 1000);

// Deal close timer - runs every 10s, if there's an active bid, start a 5s timeout
setInterval(function() {
  if (buyer_available_0 == 1) {
    Card_0 = setTimeout(function() {
      io.emit('accept_deal_0', {
        producer_address: producer_address_list[0],
        bid: asking_bid[0],
        consumer_address: consumer_address_list[0]
      });
      console.log('[MAIN SERVER] Deal 0 closed: producer=' + producer_address_list[0] + ', consumer=' + consumer_address_list[0] + ', bid=' + asking_bid[0]);
      producer_address_list[0] = ""; consumer_address_list[0] = "";
      asking_bid[0] = 0; energy_token[0] = "";
      buyer_available_0 = 0; seller_available_0 = 0;
    }, 5000);
  }
  if (buyer_available_1 == 1) {
    Card_1 = setTimeout(function() {
      ...
      producer_address_list[1] = ""; consumer_address_list[1] = "";
      asking_bid[1] = 0; energy_token[1] = "";
      buyer_available_1 = 0; seller_available_1 = 0;
    }, 5000);
  }
  if (buyer_available_2 == 1) {
    Card_2 = setTimeout(function() {
      ...
      producer_address_list[2] = ""; consumer_address_list[2] = "";
      asking_bid[2] = 0; energy_token[2] = "";
      buyer_available_2 = 0; seller_available_2 = 0;
    }, 5000);
  }
  if (buyer_available_3 == 1) {
    Card_3 = setTimeout(function() {
      ...
      producer_address_list[3] = ""; consumer_address_list[3] = "";
      asking_bid[3] = 0; energy_token[3] = "";
      buyer_available_3 = 0; seller_available_3 = 0;
    }, 5000);
  }
  if (buyer_available_4 == 1) {
    Card_4 = setTimeout(function() {
      ...
      producer_address_list[4] = ""; consumer_address_list[4] = "";
      asking_bid[4] = 0; energy_token[4] = "";
      buyer_available_4 = 0; seller_available_4 = 0;
    }, 5000);
  }
  if (buyer_available_5 == 1) {
    Card_5 = setTimeout(function() {
      ...
      producer_address_list[5] = ""; consumer_address_list[5] = "";
      asking_bid[5] = 0; energy_token[5] = "";
      buyer_available_5 = 0; seller_available_5 = 0;
    }, 5000);
  }
}, 10000);

http.listen(4000, function() {
  console.log('[MAIN SERVER] Marketplace listening on http://localhost:4000');
});